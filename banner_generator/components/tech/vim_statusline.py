...
class Component(BaseComponent):
    config_schema = {}
    z_index = 100
    section_name = 'vim_statusline'
    description = 'Vim‑style statusline segments'
    ...
    def render(self) -> str:
        ...
        for i, seg in enumerate(segments):
            ...
            parts.append(f'''<text x="{cur_x + seg_w / 2}" y="{y + rect_h / 2 + font_size / 3}" font-family="'JetBrains Mono', monospace" font-size="{font_size}" fill="{fg}" text-anchor="middle">{xml_escape(txt)}</text>''')
            cur_x += seg_w + 2
