...
class Component(BaseComponent):
    ...
    def _get_live_metrics(self, entries_config):
        ...
        # Basic system info (always available without psutil)
        ...
        # Replace placeholders in each entry string
        live_entries = []
    ...
    def render(self) -> str:
        ...
        entry_templates = cfg.get("entries", [
            "{user}@{hostname}",
            "Distro: {distro}, WM: {wm}",
            "CPU: {cpu_percent}",
        ...
        # New configurable bar offset – default 200px to avoid overlap
        ...
        for entry in entries:
            parts.append(
                ...
                f'font-size="{font_size}" fill="{label_color}">{xml_escape(entry)}</text>'
            )
            if show_bars and '%' in entry:
                ...
                if match:
                    ...
                    # Vertically center the bar within the line
                    bar_y = cur_y - (font_size / 2) - 3   # 3 = half the bar height (6px)
