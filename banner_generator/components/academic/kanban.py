    ...
    def render(self) -> str:
        ...
        for i, col_title in enumerate(columns):
            ...
            for j, card in enumerate(cards[start_idx:end_idx]):
                ...
                parts.append(f'''<text x="{cur_x + col_w / 2}" y="{card_y + card_h / 2 + 4}" text-anchor="middle" font-family="'JetBrains Mono', monospace" font-size="{font_size}" fill="{text_color}">{xml_escape(card)}</text>''')
            cur_x += col_w + gap
