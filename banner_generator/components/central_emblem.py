...
class Component(BaseComponent):
    ...
    def render(self) -> str:
        ...
        style = cfg.get("style", "shield")   # shield, circle, diamond, star
        ...
        # Shield path (classic medieval)
        if style == "shield":
            path = (f'M{x},{y - size} L{x + size},{y - size * 0.4} '
                    f'L{x + size * 0.5},{y + size * 0.6} L{x},{y + size} '
                    f'L{x - size * 0.5},{y + size * 0.6} L{x - size},{y - size * 0.4} Z')
        elif style == "diamond":
            path = (f'M{x},{y - size} L{x + size},{y} '
                    f'L{x},{y + size} L{x - size},{y} Z')
        else:  # circle
