...
# banner_generator/components/latex_source.py
...
class Component(BaseComponent):
    ...
    def _tokenize_fallback(self, line: str) -> list[tuple[str, str, bool]]:
        ...
        while i < length:
            if line[i] == '%':
                ...
                # plain text until next control character
                j = i
    ...
    def render(self) -> str:
        ...
        code = cfg.get("code", [r"\documentclass{article}", r"\begin{document}", r"Hello world", r"\end{document}"])
        ...
        for i, line in enumerate(code):
            ...
            if show_line_numbers:
                parts.append(
                    f'<text x="{x + 8}" y="{line_y + line_h - 4}" font-family="\'JetBrains Mono\', monospace" '
                    f'font-size="{font_size-1}" fill="{line_no_color}" text-anchor="end">{i+1}</text>'
                )
