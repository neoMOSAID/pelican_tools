...
def _parse_svg_inner(raw_svg: str):
    ...
    if raw_svg.lower().startswith("<svg"):
        match = re.search(r'viewBox\s*=\s*"([\d\.\s]+)"', raw_svg)
        if match:
            parts = match.group(1).split()
            if len(parts) == 4:
                w = float(parts[2]); h = float(parts[3])
        else:
            match_w = re.search(r'width\s*=\s*"([\d\.]+)"', raw_svg)
            match_h = re.search(r'height\s*=\s*"([\d\.]+)"', raw_svg)
            if match_w: w = float(match_w.group(1))
            if match_h: h = float(match_h.group(1))
    ...
    # Remove opacity attrs (we control opacity externally)
    inner_raw = re.sub(r'\sopacity\s*=\s*"[^"]*"', "", inner_raw)
...
class Component(BaseComponent):
    ...
    def render(self) -> str:
        # Always on by default; can be disabled via components.show_watermark=false
