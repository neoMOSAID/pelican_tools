...
class Component(BaseComponent):
    config_schema = {}
    z_index = 100
    section_name = 'author_plate'
    description = 'Styled author name with ornaments'
    ...
    def render(self) -> str:
        ...
        if show_rule:
            ...
            rule_y = y - 15
            rule_parts = [f'<line x1="{x - rule_width // 2}" y1="{rule_y}" x2="{x + rule_width // 2}" y2="{rule_y}" stroke="{color}" stroke-width="1.5" opacity="0.6"/>', f'<circle cx="{x - rule_width // 2 - 6}" cy="{rule_y}" r="3" fill="{color}" opacity="0.8"/>', f'<circle cx="{x + rule_width // 2 + 6}" cy="{rule_y}" r="3" fill="{color}" opacity="0.8"/>']
