...
# banner_generator/components/network_diagram.py
...
class Component(BaseComponent):
    ...
    def render(self) -> str:
        ...
        parts = []
        # Draw lines between consecutive nodes (simple chain)
        ...
        for (cx, cy), label in zip(centers, nodes):
            ...
            parts.append(
                ...
                f'font-family="\'JetBrains Mono\', monospace" font-size="12" fill="{text_color}">{xml_escape(label)}</text>'
            )

        # Calculate used height: find max y + radius
