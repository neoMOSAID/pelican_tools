...
# banner_generator/components/database_relations_tables.py
...
class Component(BaseComponent):
    ...
    def render(self) -> str:
        ...
        for table in tables:
            ...
            # Flatten top corners if needed – simple
            parts.append(f'<rect x="{cur_x}" y="{y + header_h//2}" width="{table_w}" height="{header_h//2}" fill="{header_bg}" />')

            # Header text
            parts.append(
                f'<text x="{cur_x + table_w/2}" y="{y + header_h/2 + 4}" text-anchor="middle" '
                f'font-family="\'JetBrains Mono\', monospace" font-size="{font_size}" font-weight="bold" fill="{header_fg}">{xml_escape(name)}</text>'
            )
            ...
            for i, col in enumerate(cols):
                ...
                parts.append(
                    f'<text x="{cur_x + 10}" y="{row_y + row_h/2 + 4}" '
                    f'font-family="\'JetBrains Mono\', monospace" font-size="{font_size}" fill="{self.col.get("db_text", "#cdd6f4")}">{xml_escape(col)}</text>'
                )

            # Optional: draw arrow between tables if more than one and relation defined
            if len(tables) > 1 and table != tables[-1]:
                next_x = cur_x + table_w + gap
                arrow_y = y + total_h // 2
                parts.append(
                    f'<line x1="{cur_x + table_w}" y1="{arrow_y}" x2="{next_x - 10}" y2="{arrow_y}" '
                    f'stroke="{border}" stroke-width="2" marker-end="url(#arrowhead)" />'
                )
            cur_x += table_w + gap
            max_h = max(max_h, total_h)

        # Marker definition for arrow – include in defs
