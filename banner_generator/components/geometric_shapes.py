...
class Component(BaseComponent):
    ...
    def render(self) -> str:
        ...
        # ---- Horizontal bars (e.g., top / bottom accents) ----
        ...
        # ---- Diagonal lines / zigzag ----
        if cfg.get("diagonal_lines", False):
            # Example: a few subtle diagonal lines from bottom-left to top-right
            for i in range(0, h, 80):
                parts.append(
                    f'<line x1="0" y1="{i}" x2="{w}" y2="{i+40}" '
                    f'stroke="{c.get("diagonal_stroke", "#d4af37")}" stroke-width="1" opacity="0.1" />'
                )
