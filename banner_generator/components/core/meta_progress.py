...
class Component(BaseComponent):
    config_schema = {}
    z_index = 70
    section_name = 'meta_progress'
    description = 'Metadata line and optional progress bar'
    ...
    def render(self) -> str:
        ...
        meta_y = base_y + int(meta_cfg.get('y_offset', 20))
        ...
        parts = []
        if meta_text:
            parts.append(f'''<text x="{meta_x}" y="{meta_y}" font-family="'JetBrains Mono', 'SF Mono', monospace" font-size="{font_size}" fill="{self.col.get('meta', '#6c7086')}" font-weight="600" text-anchor="{anchor}">{xml_escape(str(meta_text))}</text>''')
        if show_progress and progress_text:
            ...
            parts.append(f'''<rect x="{meta_x}" y="{bar_y}" width="{bar_width}" height="6" rx="3" fill="{self.col.get('progress_bar_bg', '#313244')}" />''')
            parts.append(f'<rect x="{meta_x}" y="{bar_y}" width="{pw}" height="6" rx="3" fill="{fill}" />')
            parts.append(f'''<text x="{meta_x}" y="{bar_y + 24}" font-family="'JetBrains Mono', monospace" font-size="13" fill="{self.col.get('meta', '#6c7086')}">{xml_escape(str(progress_text))}</text>''')
